...
router.get('/sanpham', async (req, res) => {
  try {
    // Chỉ lấy các thể loại không bị xóa mềm
      ...
      theloai.map(async tl => {
        const sanpham = await Promise.all(
          tl.chitietsp.map(async sp => {
            // Chỉ lấy chi tiết sản phẩm không bị xóa mềm
            const sp1 = await Sp.ChitietSp.findOne({
              _id: sp._id, 
              isDeleted: false  // Thêm điều kiện này để lọc sp bị xóa mềm
            })
            
            // Nếu không tìm thấy sản phẩm hoặc đã bị xóa mềm
            if (!sp1) return null;
        ...
        // Lọc bỏ các sản phẩm null (đã bị xóa mềm)
        const filteredSanpham = sanpham.filter(Boolean);
        
        // Nếu thể loại không có sản phẩm nào để hiển thị, trả về null
        ...
        return {
          ...
          khuyenmai: tl.khuyenmai,
          sanpham: filteredSanpham
        }
      })
    )
    
    // Lọc bỏ các thể loại null (không có sản phẩm)
    const filteredTheloai = theloaijson.filter(Boolean);
    
    res.json(filteredTheloai)
  } catch (error) {
    console.log(error)
...
router.get('/get-all-products', async (req, res) => {
  try {
    ...
    // Lấy tất cả sản phẩm không bị xóa (isDeleted = false)
    const allProducts = await Sp.ChitietSp.find({ isDeleted: false }).lean();
